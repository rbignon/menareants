...
idx=0
for i in ${entries}; do
  lines[$idx]=`grep -n "^Index: $i$" ${infile} | cut -d : -f 1`
  files[$idx]=`echo $i`
  idx=`expr ${idx}+1`
done
...
#  echo "	* ${files[idx]}: "
  ...
  for i in $chlines; do
    ...
    if test x"${char}" = x"+"; then
      ...
      grep -n "::.*(.*)" $filename 2>/dev/null | egrep -v ';|>|\.|while|if\(|/\*|\*/|=' > .tmp
...
cat ChangeLog >> $outfile

mv ChangeLog .ChangeLog.sav
mv $outfile ChangeLog
